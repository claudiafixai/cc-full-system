---
name: pr
description: Fix all open PR review threads (BugBot + CodeRabbit), reply inline to every thread, resolve them, and enable auto-merge. Pass a PR number or it will find the most recent open PR. Examples: /pr 114, /pr comptago 114, /pr
---
...
- `/pr 114` — targets PR #114 in the most likely repo (comptago if in that dir)
- `/pr comptago 114` — targets comptago-assistant PR #114
- `/pr viralyzio 23` — targets viralyzio PR #23
- `/pr spa 45` — targets spa-mobile PR #45
...
gh pr list --repo claudiafixai/comptago-assistant --state open --json number,title --limit 1
gh pr list --repo claudiafixai/viralyzio --state open --json number,title --limit 1
gh pr list --repo claudiafixai/spa-mobile --state open --json number,title --limit 1
```
